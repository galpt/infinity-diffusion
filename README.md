...
A ComfyUI sampler and scheduler that brings out what the model already drew: crisp line art, natural skin texture, visible fabric weave, and calm flat areas — all in a single pass, no configuration needed.
...
Restart ComfyUI and pick `infinity` in both the sampler and the scheduler dropdowns. 25 steps with CFG 7.0 is a good starting point on SD 1.5, SDXL, and flow models. Distilled models (4–8 steps, like Krea 2 Turbo) bypass all enhancements automatically — nothing to configure.
